...
- `lib/screens/about_arena_matriks_screen.dart` — static "About Pusat Tuisyen Arena Matriks" page, reachable pre-login from both `WelcomeScreen` and `LoginScreen`. Deliberately has zero Firestore/Auth dependency (no live data) so it doesn't need a public-read security rule — content is static placeholder copy, not the stakeholder's real address/contact details (those weren't supplied; update the text directly in this file once available).
...
Done: login/role routing + register + session persistence (AuthGate, no repeated login on app restart), real-time chat, office hour lock (+ debug bypass + overtime mode + manual On-Duty/Off-Duty toggle), Firestore rules, chat list (tabs + unread badges + read receipts), group chat (create/manage members), user search + profile view, Admin (manage users/subjects, link/unlink Parent-Student, Reports, full user deletion via Cloud Function), file upload/attachments (Storage live, rules deployed), OS-level unread app badge, quick reply chips, Interactive Quiz (Live Session + Self-Paced), Class Performance Overview + Warning Letter, Attendance (take attendance + student overview), Parent Module (chat dashboard, Child Overview, Warning Letters inbox), Push Notifications (Cloud Functions deployed and live, Android custom sound), Settings (profile/password/push toggle/logout/self-delete account, Teacher Leave dates, Notification Sound), Schedule Message available to all roles (not Teacher-only), Android release APK with TuturEdu launcher icon, About page, URL phishing detection in chat.
